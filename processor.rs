...
impl<'a> Processor {
    ...
    pub fn process_ticket_purchase(
        ...
        if *holding_wallet_account.key != lottery_data.holding_wallet {
            ...
            return Err(LotteryError::InvalidColattoAccount.into());
        }
    ...
    pub fn process_update_colatto_wallets(
        program_id: &Pubkey,
        accounts: &[AccountInfo],
        holding_wallet: Pubkey,
        rewards_wallet: Pubkey,
        slot_holders_rewards_wallet: Pubkey,
        colatto_labs_wallet: Pubkey,
    ) -> ProgramResult {
        let accounts_iter = &mut accounts.iter();
        let lottery_data_account = next_account_info(accounts_iter)?;
...
mod test {
    ...
    fn test_reward_winners() {
        ...
        // BadCase: Bad colatto reward account
        let fake_colatto_labs_wallet = Pubkey::new_unique();
        assert_eq!(
            Err(LotteryError::InvalidcolattoAccount.into()),
            do_process(
                crate::instruction::reward_winners(
                    &program_id,
                    &lottery_key,
    ...
    #[test]
    fn test_update_colatto_wallets() {
        let program_id = id();
        let lottery_id = 112233;
        let lottery_key = Pubkey::new_unique();
        let mut lottery_acc = SolanaAccount::new(
        ...
        assert_eq!(
            Err(LotteryError::NotInitialized.into()),
            do_process(
                crate::instruction::update_colatto_wallets(
                    &program_id,
                    &new_holding_wallet,
                    &new_rewards_wallet,
        ...
        assert_eq!(lottery.colatto_labs_wallet, colatto_labs_wallet);
